...
class ResUNet30_Base(nn.Module, Base):
    def __init__(self, input_channels, output_channels, target_sources_num, soundbeam=None,
    ...
    def feature_maps_to_wav(
        ...
        out_real = self.out_conv(out_real)  # out_conv converts input_channels (4) to output_channels (1), kernel_size=1, (batch_size, 4, ...) --> (batch_size, 1, ...)
        ...
        out_real = out_real.reshape(shape)  # (batch_size, 1, 2001, 1025)
        out_imag = out_imag.reshape(shape)  # (batch_size, 1, 2001, 1025)

        # ISTFT.
        x = self.istft(out_real, out_imag, audio_length)  # x.shape = (batch_size, 320_000)
        ...
        if self.target_sources_num == 1:
            ...
            )  # waveform.shape = (batch_size, 1, 320_000)
    ...
    def forward(self, mixtures, film_dict, source_mask, weighted_avg=None, time_film_dict=None):
        ...
        x = self.after_conv(x12) # x.shape = (bs, input_channels * self.K * self.target_sources_num, 2016, 1024)
        # input_channels * self.K * self.target_sources_num = 4 * 3 * 1 = 12
...
class ResUNet30(nn.Module):
    def __init__(self, input_channels, output_channels, condition_size, target_sources_num,
                 label_len, soundbeam=None, use_time_film=False, time_film_mode: str = "additive",
                 binarize_sed_preds: bool = False, channels_last=False, load_pretrained=True,
                 hop_size: int = 320, window_size: int = 2048, **kwargs):
        super(ResUNet30, self).__init__()

        print("audiosep_resunet.resunet.py: ResUNet30.__init__()")
        ...
        if load_pretrained:
            ...
            if pre_conv_key in state_dict and state_dict[pre_conv_key].shape[1] == 1:
                ...
                state_dict[pre_conv_key] = state_dict[pre_conv_key].repeat(1, 4, 1, 1) / 4
            ...
            assert all([k.startswith("base.out_conv.") or k.startswith("label_embedding.")
                        or k.startswith("base.downsample_") or k.startswith("base.weight_") for k in missing])
    ...
    def forward_inference(self, input_dict, nsources=3, label_len=18):
