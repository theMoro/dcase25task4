...
class PatchEmbed(torch.nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x)  # (bs, 1, 80, 1001) -> (bs, 768, 5, 62)
        if self.flatten:
            x = x.flatten(2).transpose(1, 2)  # BCHW -> B(HW)C --> BNC, (bs, 310, 768) --> 310 = freq_bins * time_frames
        x = self.norm(x)
        return x
...
class LocalViT(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_encoder(self, x, return_features=False):
        features = []

        x = self.patch_embed(x)  # (5, 310, 768)
        if return_features:
            x_ = F.pad(x, (0, 0, 1, 0), mode='constant', value=0)
            features.append(x_)  # Add patch_embed output
...
class PortableM2D(torch.nn.Module):
    ...
    def to_normalized_feature(self, batch_audio):
        x = self.to_log_mel_spec(batch_audio)  # 32 x 1 x 80 x 1001
        x = self.normalize_batch(x)
        return x

    def encode_lms(self, x, average_per_time_frame=False, return_features=False):
        # x.shape = (bs, 1, 80, 1001) --> 80 fbins, 1001 frames (time axis)
        patch_fbins = self.backbone.grid_size()[0]  # 5
        unit_frames = self.cfg.input_size[1]  # 1001
        patch_frames = self.backbone.patch_size()[1]  # 16
        embed_d = self.backbone.patch_embed.proj.out_channels  # 768
        n_chunk = (x.shape[-1] + unit_frames - 1) // unit_frames  # 1
        ...
        nr_of_blocks = len(self.backbone.blocks) + 1  # +1 for the patch embedding / conv block

        # Initialize list of lists for each block's features
        all_features = [[] for _ in range(nr_of_blocks)]
        features = []
        ...
        if self.cfg.flat_features:
            ...
            for i in range(n_chunk):
                if return_features:
                    emb, features = self.backbone.forward_encoder(x[..., i*unit_frames:(i+1)*unit_frames], return_features=True)
                else:
                ...
                if return_features:
                    # Transform features like emb
                    for block_idx, feature in enumerate(features):
                        feature = feature[..., 1:, :]  # remove cls token
                        if average_per_time_frame:
                            feature = rearrange(feature, 'b (f t) d -> b t d f', f=patch_fbins, d=embed_d).mean(-1)
                        # else, keep as [b, f*t, d]
                        all_features[block_idx].append(feature)
            ...
            for i in range(n_chunk):
                ...
                if return_features:
                    ...
                    for block_idx, feature in enumerate(features):
                        feature = feature[..., 1:, :]  # remove cls token
                        feature = rearrange(feature, 'b (f t) d -> b t (f d)', f=patch_fbins, d=embed_d)
                        all_features[block_idx].append(feature)

        # concatenate embedding chunks in the time axis
        x = torch.cat(embeddings, dim=-2)  # axis=-2

        # Concatenate features for each transformer block
        if return_features:
            for block_idx in range(nr_of_blocks):
                all_features[block_idx] = torch.cat(all_features[block_idx], dim=1)
    ...
    def encode(self, batch_audio, average_per_time_frame=False, return_features=False):
        x = self.to_normalized_feature(batch_audio)
        return self.encode_lms(x, average_per_time_frame=average_per_time_frame, return_features=return_features)
