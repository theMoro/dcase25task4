...
class PortableM2D(torch.nn.Module):
    ...
    def forward_mel(self, batch_mel, average_per_time_frame=False, return_features=False):
        x = self.encode_lms(batch_mel, average_per_time_frame=average_per_time_frame, return_features=return_features)
        if hasattr(self, 'head'):
            x = x.mean(1)  # B, D
            x = self.head_norm(x.unsqueeze(-1)).squeeze(-1)
            x = self.head(x)
